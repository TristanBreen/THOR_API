  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`card bg-gradient-to-br ${getSeverityColor(pnesAnalysis.pnes_likelihood_score)} p-8`}>
        <div className="flex items-center justify-between gap-6">
          <div className="flex-1">
            <h3 className="text-2xl font-bold text-white mb-3">✓ PNES Diagnosis Confirmed</h3>
          ...
          <div className="text-right">
            <div className="text-base font-semibold text-white/80 mb-2">Symptom Severity Index</div>
            <div className="text-5xl font-bold text-white">{pnesAnalysis.pnes_likelihood_score}</div>
        ...
        <div className="mt-6 bg-white/10 rounded-lg p-5 border border-white/20">
          <p className="text-white font-semibold text-base mb-2">Clinical Status:</p>
          ...
          <div className="space-y-4">
            {pnesAnalysis.risk_factors.map((factor, idx) => (
              <div key={idx} className="border-l-4 border-purple-400 bg-purple-50 rounded-lg p-5 hover:bg-purple-100 transition-colors">
                ...
                <p className="text-gray-700 text-sm mb-3 leading-relaxed">{factor.description}</p>
                <div className="bg-white rounded p-3 text-sm">
                  <p className="text-gray-800">
                    <span className="font-bold text-purple-700">How it manifests in PNES:</span> {factor.pnes_relevance}
                  </p>
                </div>
              </div>
            ))}
      ...
      {pnesAnalysis.recommendations.length > 0 && (
        <div className="card border-l-4 border-emerald-400 bg-emerald-50 p-8">
          <h4 className="font-bold text-emerald-900 mb-6 text-xl flex items-center gap-3">
            <svg
              className="w-6 h-6"
              fill="none"
              stroke="currentColor"
          ...
          <ul className="space-y-3">
            {pnesAnalysis.recommendations.map((rec, idx) => (
              <li key={idx} className="flex gap-4 text-gray-700 text-base leading-relaxed">
                <span className="text-emerald-600 font-bold text-lg flex-shrink-0 mt-0.5">→</span>
                <span>{rec}</span>
              </li>
            ))}
          </ul>
