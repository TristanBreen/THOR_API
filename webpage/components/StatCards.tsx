    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-slideUp">
      {cards.map((card, index) => {
        ...
        const bgColor = card.alert
          ? 'from-rose-900/20 to-rose-800/10'
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <p className="text-xs font-bold uppercase tracking-wider text-sage-500 group-hover:text-sage-400 transition-colors">
                  {card.label}
                </p>
              </div>
              <Icon className={`w-5 h-5 ${card.alert ? 'text-rose-500' : 'text-sage-500'}`} />
            </div>

            <div className="mb-2">
              <p
                className={`text-3xl font-bold ${
                  card.alert ? 'text-rose-400' : 'text-slate-100'
                }`}
              >
                {card.value}
              </p>
